...
Run /simplify before every commit that contains code changes. This is a HARD REQUIREMENT -- do not skip it, do not commit first and simplify later. The flow is always: write code → run e2e tests → /simplify → fix issues → re-test → commit. IMPORTANT: /simplify is a skill that launches THREE review agents in parallel (code reuse, code quality, efficiency). Do NOT substitute a single manual review agent -- always invoke the /simplify skill which runs the full three-agent review.
